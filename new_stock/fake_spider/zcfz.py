...
ID_NAME = const.ZCFZ_KEYWORD.ID_NAME
DB_NAME = const.ZCFZ_KEYWORD.DB_NAME
COLLECTION_HEAD = const.ZCFZ_KEYWORD.COLLECTION_HEAD
KEY_NAME = const.ZCFZ_KEYWORD.KEY_NAME
NEED_TO_NUMBER = const.ZCFZ_KEYWORD.NEED_TO_NUMBER
DATA_SUB = const.ZCFZ_KEYWORD.DATA_SUB
NEED_TO_DECODE = const.ZCFZ_KEYWORD.NEED_TO_DECODE
...
headers = {
  ...
  'Referer': 'http://data.eastmoney.com/bbsj/zcfz/000636.html',
  ...
  def parse_page(self, json, mapping):
    ...
    try:
      ...
      for item in json:
        ...
        #计算
        if isinstance(one_stock[KEY_NAME['accountrec']], float) and isinstance(one_stock[KEY_NAME['sumasset']], float):
          one_stock[KEY_NAME['accountrec_sumasset']] = one_stock[KEY_NAME['accountrec']] / one_stock[KEY_NAME['sumasset']]
        if isinstance(one_stock[KEY_NAME['inventory']], float) and isinstance(one_stock[KEY_NAME['sumasset']], float):
          one_stock[KEY_NAME['inventory_sumasset']] = one_stock[KEY_NAME['inventory']] / one_stock[KEY_NAME['sumasset']]
