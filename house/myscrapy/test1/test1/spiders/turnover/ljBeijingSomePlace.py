...
class Spider(scrapy.Spider):
  name = 'lianjia-cj-bj-lsq'
  allowed_domains = [
    'bj.lianjia.com',
  ]
  start_urls = [
    'https://bj.lianjia.com/chengjiao/lishuiqiao1/'
    #'https://bj.lianjia.com/chengjiao/pinggu/'
  ]
  head = 'https://bj.lianjia.com'
  dbName = 'house-cj'
  collectionName = 'beijing-lsq'
  ...
  def parseOne(self, one, district):
    ...
    title = ''.join(one.xpath('./div/div[1]/a/text()').extract()).strip()
    title = title.split()
    if len(title) > 0:
      oneOut['title'] = title[0]
      if len(title) > 1:
        oneOut['houseType'] = title[1]
        if len(title) > 2:
          oneOut['square'] = String2Number(title[2])
    ...
    try:
      ...
      oneOut['dealDate'] = datetime.datetime.strptime(oneOut['dealDate'], '%Y.%m.%d')
