...
class HooksasyncExtension(object):
    ...
    def from_crawler(cls, crawler):
        logging.warning("HooksasyncExtension from_crawler")
        return cls(crawler)

    def __init__(self, crawler):
        logging.warning("HooksasyncExtension Constructor called")
    ...
    def engine_started(self):
        logging.warning("HooksasyncExtension, signals.engine_started fired")

    def engine_stopped(self):
        logging.warning("HooksasyncExtension, signals.engine_stopped fired")

    def spider_opened(self, spider):
        logging.warning("HooksasyncExtension, signals.spider_opened fired")

    def spider_idle(self, spider):
        logging.warning("HooksasyncExtension, signals.spider_idle fired")

    def spider_closed(self, spider, reason):
        logging.warning("HooksasyncExtension, signals.spider_closed fired")

    def spider_error(self, failure, response, spider):
        logging.warning("HooksasyncExtension, signals.spider_error fired")

    def request_scheduled(self, request, spider):
        #logging.warning("HooksasyncExtension, signals.request_scheduled fired")
        logging.warning("[signals.request_scheduled]the url %s"%(request.url))

    def response_received(self, response, request, spider):
        #logging.warning("HooksasyncExtension, signals.response_received fired")
        logging.warning("[signals.response_received] the status and url [%d]  %s" % (response.status, response.url))

    def response_downloaded(self, response, request, spider):
        logging.warning("HooksasyncExtension, signals.response_downloaded fired")

    def item_scraped(self, item, response, spider):
        #logging.warning("HooksasyncExtension, signals.item_scraped fired")
        pass

    def item_dropped(self, item, spider, exception):
        logging.warning("HooksasyncExtension, signals.item_dropped fired")

    @classmethod
    def from_settings(cls, settings):
        logging.warning("HooksasyncExtension from_settings")
